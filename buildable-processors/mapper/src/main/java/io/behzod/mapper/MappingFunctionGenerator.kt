            ...
            if (matchingConstructorArguments.isEmpty()) {
                logger.warn(
                    message = "\n\nNo matching arguments for mapping from class `$sourceClassName` to `$targetClassName`. \n" +
                            "You can instead remove the argument `$sourceClassName::class` from the `$BuildableMapperConstants.MAPPER_ANNOTATION_NAME` annotation in class `$targetClassName` " +
